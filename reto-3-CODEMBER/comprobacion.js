...
const llavesArr = llaves.replace(/-/g, " ").replace(/:/g, "").replace(/\n/g, " ").split(" ")
...
const countWordsinString = (word, stringToReview) => {
    const stringArr = stringToReview.split("")
    let count = 0
...
arrKeys.map( key => {
    let min = key[0], max = key[1], word = key[2], keyToReview = key[3]

    const count = countWordsinString(word, keyToReview)

    if (count >= min && count <= max) {
        console.log(`La llave ${keyToReview} SÍ cumple con los requisitos`)
    } else {
        noPass++
        console.log(`La llave ${keyToReview} NO cumple con los requisitos <===== ${noPass}`)
    }
})