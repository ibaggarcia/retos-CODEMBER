...
const arrMensaje = MENSAJE.split(' ')

const countAnimals = {}

arrMensaje.map( animal => {
    if (animal in countAnimals) {
        countAnimals[animal] = countAnimals[animal] + 1
    } else {
        countAnimals[animal] = 1
    }
})

let finalMessage = JSON.stringify(countAnimals).replace('{', '').replace(/"/g, '').replace(/:/g, '').replace('}', '')

console.log(finalMessage.split(',').join(''))
